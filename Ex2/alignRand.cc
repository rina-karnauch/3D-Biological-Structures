...
//
//vector<RigidTrans3>
//getAllTransformations(const vector<Triangle> &mulModel,
//					  const vector<Triangle> &mulTarget)
//{
//
//	vector<RigidTrans3> transformations;
//	for (auto &tr : mulTarget)
//	{
//		for (auto &mo : mulModel)
//		{
//			RigidTrans3 current = getTransformation(mo, tr);
//
...
//	}
//
//	// reading the model with CA's and also reading the full model itself.
...
//
//	for (int i = 0; i < molModel.size() - 3; i++)
//	{
//		Triangle mol = Triangle(molModel[i], molModel[i+1], molModel[i+2]);
//		for (int j = 0; j < molTarget.size() - 3; j++)
//		{
//			Triangle tr = Triangle(molTarget[i], molTarget[i+3], molTarget[i+2]);
//			RigidTrans3 currentTransformation = mol | tr;
//			Match match = Match(currentTransformation);
//
//			for (unsigned int k = 0; k < molModel.size(); k++)
//			{
//
//				Vector3 mol_atom = currentTransformation.rotation() * molModel[k].position(); //
//				// rotate
//
//				// find close target molecule atoms using the hash
//				HashResult<int> result;
